...
def get_question():
    for page in range(0, 5):
        ...
        req = urllib2.Request(url=exam_url, data=urllib.urlencode(exam_post_data), headers=exam_head)
...
def get_query_ans(words):
        query = urllib.quote(words)
        req = urllib2.Request(url=answer_url + query, headers=answer_head)
        res = urllib2.urlopen(req)
        bsObj = BeautifulSoup(res.read(), "html.parser")
        ...
        req2 = urllib2.Request(url=ans_url, headers=answer_head)
        ...
        if len(ans_list) == 0:
                return {}
        return {"desc": desc_list[0].text.encode("utf8").strip(), "answer": ans_list[0].text.encode("utf8").strip().replace('\n', '')}
